...
// addBuyingProduct adds a product to the user's buying list.
func addBuyingProduct(c *gin.Context) {
	user := c.Param("user_id")
	...
	type buyingProduct struct {
		ProductID int `json:"product_id" binding:"required" db:"fk_product_id"`
	}

	var product buyingProduct
	...
	query := "INSERT INTO Buying_Product (fk_product_id, fk_user_id) VALUES($1,$2) RETURNING fk_product_id"
	err = pgxscan.Get(c, dbPool, &product, query, product.ProductID, user)
...
// getBuyingProducts returns all the products that the user is buying.
func getBuyingProducts(c *gin.Context) {
	user := c.Param("user_id")
	if checkIfUserExist(c, user) == false {
		c.JSON(http.StatusNotFound, gin.H{"error": "User does not exist"})
		return
	...
	var buyingProducts []*Product

	query := "SELECT * from Product WHERE product_id IN (SELECT fk_product_id FROM Buying_Product WHERE fk_user_id = $1)"
	err := pgxscan.Select(c, dbPool, &buyingProducts, query, user)
	...
	c.JSON(http.StatusOK, buyingProducts)
}

// deleteBuyingProduct removes a product from the user's buying list.
func deleteBuyingProduct(c *gin.Context) {
	userID := c.Param("user_id")
	productID := c.Param("product_id")
	...
	// Check if the product is in the user's buying list.
	var buyingProductID int

	query := "SELECT fk_product_id FROM Buying_Product WHERE fk_user_id = $1 AND fk_product_id = $2"
	err := pgxscan.Get(c, dbPool, &buyingProductID, query, userID, productID)

	if err != nil {
		if err.Error() == "no rows in result set" {
			c.JSON(http.StatusNotFound, gin.H{"error": "Product is not in the user's buying list"})
			return
		}
	...
	query = "DELETE FROM Buying_Product WHERE fk_user_id = $1 AND fk_product_id = $2"
	_, err = dbPool.Exec(c, query, userID, productID)
